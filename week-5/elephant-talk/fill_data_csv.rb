...
class FillData
  $file_products_customers = "products_customers.sql"
  $file_transactions = "transactions.sql"
  def products_customers
    ...
    customer_state = []
    customer_country = []
    file =  File.open($file_products_customers, "w")
    ...
    products_name.each_with_index do |product, index|
      file.puts "INSERT INTO products(name, days, price) VALUES('#{product}', '#{products_days[index]}', '#{products_price[index]}');"
    end
    
    customer_name.each_with_index do |customer, index|
      file.puts "INSERT INTO customers(name, city, state, country) VALUES('#{customer}', '#{customer_city[index]}', '#{customer_state[index]}', '#{customer_country[index]}');"
    end
  end
  
  def transactions
    customer_counter = 0
    file =  File.open($file_transactions, "w")
    CSV.foreach("sales.csv", headers: true) do |row|
      product = row[1]
      case product
      when "Caribbean Tour"
